package com.amorphous.flame.fields
{
	 ...
	 * This class is used to draw out grids that do not require fluid dynamics, especially
	 * the FUEL field, as we are assuming that the fuel won't leak all over the place.
	 * 
	 * All FluidSolver functions are present, but the intensive mathematic calculations have been left
	 * out of the class, allowing for simple grids to be drawn yet still have fluid qualities if need be.
	 
	/***********************************************************************
	 
	 
	/**
	 * Non-Fluid Grid class - uses similar parameters as the FluidSolver class, but doesn't actually use fluid dynamics
	 * Used to preserve consistency between the fluid and the non-fluid parameters.
	 * Modified from the AS3 port of MSAFluid
	 */
	public final class NonFluidGrid 
	{
		...
		public function NonFluidGrid(NX:int, NY:int)
		{
			setup(NX, NY);
		}
